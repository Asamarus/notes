...
import { setNotesTags } from 'store/NotesSlice';

function onSelect(tags, dispatch) {
  dispatch(setNotesTags([...tags]));
...
function Tags() {
  ...
  const onConfirm = () => {
    ...
    forEach(rows, (r) => {
      const checked = get(r, 'checked', false);
      if (checked) {
        tags.push(r['name']);
      }
    });

    onSelect(tags, dispatch);
    ...
    <>
      ...
      <ScrollArea
        ...
        <List
          ...
          onSelect={(item) => {
            const { id } = item;
            const checked = get(item, 'checked', false);

            if (isMounted()) {
              const rows = klona(state.rows);
              set(rows, `${id}.checked`, !checked);
